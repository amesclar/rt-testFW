...
String buildTestLogMsg(int testId, String which, unsigned long ticks){
  String testLogMsg = "<Test testId=\"";
  testLogMsg += String(testId);
  testLogMsg += "\"";
  testLogMsg += " which=\"";
  testLogMsg += which + "\"";
  testLogMsg += " ticks=\"";
  testLogMsg += String(ticks); // Concatenate the dynamic time value
  testLogMsg += "\"/>  ";
...
void loop() {
  ...
  if ((currentMillis >= testEndMillis) && (loopCurrent <= loopMax) && (doTest)) {
    ...
    if (testCurrent == 1) {
      ...
      logMsg(buildTestLogMsg(loopCurrent, "1min", millis()));
      ledBlink(ledPin, 500);
      ...
      logMsg(buildTestLogMsg(loopCurrent, "2min", millis()));
      ledBlink(ledPin, 500);
      ...
      logMsg(buildTestLogMsg(loopCurrent, "3min", millis()));
      ledBlink(ledPin, 500);
      ...
      logMsg(buildTestLogMsg(loopCurrent, "5min", millis()));
      ledBlink(ledPin, 500);
